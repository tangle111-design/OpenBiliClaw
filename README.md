...
## 📌 三端可编辑画像 · 编辑模式上线（2026-05-29）

- **✏️ 画像可以自己改了** —— 插件 / 移动 / 桌面三端画像页新增「编辑画像」：增删核心特质·价值观·兴趣领域·常看 UP，改写人格素描等长文。
- **🛡️ 改了不被覆盖** —— 编辑存进独立覆盖层，AI 后续自动重建画像也不会抹掉；删错了点「恢复 AI 建议」即可还原。
- **🎯 删 / 拉黑即时影响推荐** —— 标「不喜欢」的领域进入硬过滤并清理已入池相关内容，不必等下一轮学习。
- **🤝 与 AI 协作** —— 固定某段文本后，AI 有新判断会提示「AI 想更新此项」，由你决定是否采纳。
