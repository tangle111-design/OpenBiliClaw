...
11. 不同事件类型的信号强度差异（基于 metadata.signal_strength 或 event_type）：
    - event_type="favorite"（收藏）：metadata.signal_strength 通常为 1.0，表示最高信号强度，用户主动收藏，表示明确喜欢，权重应该最高（建议 weight 0.8-1.0）
    - event_type="like"（点赞）：metadata.signal_strength 通常为 0.85，表示高信号强度，用户主动点赞，表示明确喜欢，权重应该高（建议 weight 0.7-0.9）
    - event_type="follow"（关注）：metadata.signal_strength 通常为 0.6-1.0，表示中等-高信号强度，表示长期兴趣，权重中等-高（建议 weight 0.6-0.8）
    - event_type="coin"（投币）：B站特有的投币行为，表示强烈喜欢，权重应该最高（建议 weight 0.9-1.0）
    - event_type="comment"（评论）：用户主动参与讨论，表示明确兴趣，权重高（建议 weight 0.7-0.85）
    - event_type="share"（分享）：用户主动分享，表示明确喜欢且愿意传播，权重高（建议 weight 0.75-0.9）
    - event_type="view"（观看历史）：metadata.signal_strength 通常为 0.35-0.4，表示低信号强度，可能是被动观看，权重较低（建议 weight 0.3-0.5）
    - event_type="click"（点开了）：用户点击但未明确表态，权重较低（建议 weight 0.3-0.5）
    - event_type="dialogue"（聊到）：用户在对话中提到，表示中等兴趣，权重中等（建议 weight 0.5-0.7）
    - event_type="feedback"（反馈过）：用户主动反馈，根据 metadata.feedback_type 判断：
      - feedback_type="like" 或 reaction="thumbs_up"：正面反馈，权重高（建议 weight 0.7-0.9）
      - feedback_type="dislike" 或 reaction="thumbs_down"：负面反馈，用于 disliked_topics
      - feedback_type="comment"：中性反馈，权重中等（建议 weight 0.5-0.7）
    - event_type="dislike"（标记不喜欢）：负面信号，用于 disliked_topics，不提取为 interests
    - 如果 metadata.signal_strength 存在，优先使用该值作为权重参考
    - inferred_satisfaction="positive"：根据观看时长判断的正面事件，权重中等（建议 weight 0.5-0.7）
    - inferred_satisfaction="negative"：根据观看时长判断的负面事件（快速退出），用于 disliked_topics
</rules>
...
portrait 控制在 300–1000 字，核心标准是“该写的需求都写到了，不凑字数也不删骨架”。不要为了卡字数把句子压碎。
...
你是那种"在认真和放松之间反复横跳"的人。认真时密度极高——一头扎进某个问题的内部结构，不摸清脉络不罢休；放松时则完全散开，让注意力随意漫游，不给自己施加任何产出压力。这两者在你身 上并不冲突，反而构成一种节奏性的自我调节：高强度钻研消耗到一定程度，你会本能地切换到玩耍或闲逛来稀释浓度，等能量恢复后再重新潜入。对品质也有自然判断，但这条线几乎不说话，只在选择时默默起作用。
...
<output_schema>
{
  "personality_portrait": "300-1000 字的一段连续介绍(描述内在需求和为人方式,不出现具体兴趣)",
  "core_traits": ["..."],
  "cognitive_style": ["..."],
...
2. expression 必须是 50 到 150 字的中文口语表达,像朋友私聊,不像算法推荐。
   如果 source_platform 是 bilibili,可以用 B 站语境;
   xiaohongshu 用更生活化的语气;全部保持朋友感。
...
_BATCH_EXPRESSION_SYSTEM_PROMPT = (
    ...
    "3. expression 像朋友私聊。"
    "必须引用至少一个具体内容细节(标题关键词、作者特点、独特切入角度),"
    "不要说空话。\n"
...
3. 如果画像中存在 speculative_interests（猜测兴趣），至少 1 个 domain 应基于猜测兴趣的 domain 展开（可以细化或拓展，但核心方向要对应）。
...
6. why_it_might_resonate 必须先说明它对应用户的哪种认知需求、
    新颖度评分需与why_it_might_resonate中的‘共鸣逻辑’强度成反比——新颖度越高，共鸣理由必须越扎实”。
...
10. 反信息茧房：不同 domain 的 query 第一个实词（核心主题词）必须两两不同，
   禁止仅替换修饰词而保留相同核心名词；至少 4 个 domain 必须来自用户已有兴趣领域之外的全新方向（即用户画像中未出现的领域）。
...
11. 心理诉求轴多样性（核心规则，违反即视为失败）：
   ...
   定义清单（每个 domain 在 why_it_might_resonate 里**显式写出对应哪个轴**）：
     ...
     - 创造·表达·分享    ：创作教程、手工制作、艺术表达、分享日常
     - 竞争·挑战·成就    ：游戏竞技、挑战视频、极限运动、成就展示
     - 社交·连接·归属    ：社区互动、粉丝文化、圈子分享、群体活动
     - 探索·发现·新奇    ：未知领域、冷门知识、新奇事物、边缘话题
     - 放松·逃避·治愈    ：治愈系、放松音乐、冥想、减压内容
...
知识解释 / 现实观察 / 审美体验 / 人物叙事 / 技术机制 / 社会文化 / 自然科学 / 生活方式 / 创作表达 / 竞技挑战
</output_schema>
""".strip()
    user_prompt_parts = [
        "<profile_summary>",
