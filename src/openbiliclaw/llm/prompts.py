...
【第一步】看 ta 的兴趣分布,估出"生活模式占比":
   玩耍模式 / 钻研模式 / 审美模式 / 行动模式 / 倾听模式 / 闲逛模式 / other
   合计 ~100%。
...
"你的内在驱动由两股力量交替牵引：一股以趣味为基底，追求情绪能量和松弛感，对无趣的事物零容忍；另一股指向拆解和穿透，遇到陌生系统会自然启动，耐心地一层层剥开。两者在你身上各占一块，切换时不太费力，也没有明显的负罪感。近期观察到你那股拆解的力量正在从抽象层向实体层偏移，开始试探落地的可能性。"

示例 B(钻研 70% + 审美 20%):
"你的主要驱动力是认知上的完成欲——把一件事的轮廓和内部结构摸清楚，是你长期维持的常态。对感官质量也有一定的阈值，低于阈值的信号会被系统自动标记，但你不太会为此对外表达。节奏偏慢，外部节奏很难带动你，你有自己的推进节奏。认真对谈时你的输出密度会明显升高，低信息密度的闲聊不在你的默认行为区间内。"

示例 C(玩耍 80% 单一主轴):
"你的行为主线以乐趣为第一驱动力——新信息进入时会优先尝鲜，不感兴趣的部分快速放弃，不消耗额外注意力。认真是一种可选状态而非默认状态。信息摄取呈杂食特征，广度优先于深度，不作系统性的纵深追踪。社交中不制造额外的认知负荷，关系维持成本较低。"

示例 D(审美 70% + 倾听 20%):
"你对质感、调性、氛围的敏感度显著高于基线，判断'对不对劲'几乎不经过意识层，是前置到感知阶段的自动响应。对作品和人的情感纹理有感知能力，但最终落点是直觉层面的接受或排斥，而非分析性拆解。节奏偏慢，筛选标准稳定，不轻易被外部说服。这种对感官质量的长期坚持，让生活维持在比较清晰的水平。"
</positive_examples>
