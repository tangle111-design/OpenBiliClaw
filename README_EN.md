...
- 🔍 **Multi-Source Discovery Strategies** — Bilibili four strategies (Search · Related Chain · Trending · Cross-domain Explore, with a backend extension-task bridge for search cooldown fallback) + Xiaohongshu safe discovery + Douyin plugin-signed search / hot / feed + X search / For-You / followed authors, coordinated cross-platform
...
Four Bilibili strategies work in coordination, each with independent API quota; while backend Bilibili search is cooling down, the runtime has a backend task bridge that can enqueue extension search fallback tasks and accept rendered search-page results from the logged-in browser. The source layer also accepts Xiaohongshu extension-proxy signals, YouTube init signals plus a backend-direct YouTube producer, Douyin init signals / plugin-signed search / hot / feed discovery, and X (Twitter) server-side cookie-replay discovery (search / For-You / followed authors):
