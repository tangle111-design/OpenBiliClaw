...
| v0.3.x 画像输入上限放宽 | ✅ | `_recommendation_profile_summary()` 兴趣 tag 上限 10 → 30 且按 weight 降序排序后截断；`disliked_topics` 5 → 16；`_select_relevant_interests()` 的 embedding 候选池同样改为按 weight 排序取前 15（fallback「top-K by weight」语义与实现一致） |
