...
- `confirm`: 用户确认“确实不喜欢 / 需要避开这个方向”
  - 以 `preference_layer.data["disliked_topics"]` 作为持久化源头：如果候选有有效 specifics，默认只写 specifics；没有 specifics 或候选明确是 domain-level 时才写 domain，避免把“讨厌某个子形态”扩大成“讨厌整个领域”
...
2. `confirm` 将 probe 转为 confirmed，并调用统一的 dislike writeback helper 写入 flat preference 的 `disliked_topics`。写入 topic 采用上面的保守语义：specifics 优先，domain 仅在 domain-level 确认时写入。这是避雷画像的持久化源头；不要只 mutate `await soul_engine.get_profile()` 返回的快照。
