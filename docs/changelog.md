...
- **统一关键词 planner 合并调用 token 预算修复（真实模型端到端验证补强，仍默认关）**：用真实模型（deepseek）跑完整 planner 时发现合并生成是全系统输出最大的一次调用（每个 due 平台 × 至多 `gen_batch` 个词同在一个 JSON），固定 `max_tokens` 会把排在 JSON 靠后的平台**截断**、退回兴趣名兜底（实测 5 平台 ×30 词限额偏小时，youtube/twitter 退化成裸兴趣名）。两处修：① block 里给模型的每平台 `need` **收口到 `gen_batch`**——此前给的是 P3.2 动态水位（可达 `kw_cache_high×3`），而解析每平台只保留 `gen_batch`，「要 80 留 30」既浪费模型输出又顶向截断；现在「要多少＝留多少」。② 合并调用的 `max_tokens` 改为**按本轮实际要词量动态算**：`max(4096, sum(收口后 need) × 48 + 1024)`，随平台数 / `gen_batch` 自适应、慷慨留量（`max_tokens` 是天花板、按真实输出计费，放大近零成本）。真实 deepseek 复跑：五平台各满额 30 词、youtube/twitter 正确出英文、无截断；新增 `test_merged_ask_capped_at_gen_batch` / `test_merged_max_tokens_scales_with_total_ask` 锁住。全量非集成测试 2744 passed。
