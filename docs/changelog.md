...
- Windows 桌面应用改为**系统托盘常驻**：打包从控制台程序改为窗口化(`openbiliclaw.spec` `console=False`),启动后不再弹命令行窗口,而是常驻右下角系统托盘 —— uvicorn 跑在后台线程、`pystray` 托盘图标占前台,右键菜单含「打开 Web 界面 / 查看运行日志(Windows 弹实时 tail 控制台) / 退出 OpenBiliClaw」,关掉任何窗口都不会停后端,只有菜单「退出」会优雅停服(`server.should_exit`)。窗口化无 stdout,故 `entry.py` 首启即把 stdout/stderr 重定向到 `logs/desktop.log`(`print` 不再崩)、`__main__` 兜底把异常写 `logs/crash.log`。托盘仅 Windows 启用(`_should_use_tray`:frozen + `os.name=="nt"` + pystray 可用),mac/Linux 维持前台 server(`.app` 本就无控制台);`spec` 按平台只在 Windows 打包 `pystray`/`Pillow`、非 Windows 排除 pystray(避开 pyobjc)。`pyproject` packaging extra 加 `pystray`/`Pillow`,CI Windows 步骤安装它们。`tests/test_packaging_entry.py` 加托盘门控 + 日志重定向 no-op 断言。
